...
let newText5=[]
// text.split("").map((item)=>vowels.includes(item) && newText5.push(item))
// console.log(Array.from(new Set(newText5)))

let result = text.split('').filter((el,index)=>vowels.includes(el) && ! newText5.includes(el) && newText5.push(el))
console.log(result)
...
const newArr3=arr3.reduce((sum,item)=>item.region.length===6 ? sum+=item.area:sum,0)
console.log(newArr3)
